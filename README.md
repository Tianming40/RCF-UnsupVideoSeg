...
## v135–v143 — Visual consistency loss, cross-frame correlation variants, RAFT-free JEPA line (260724–260729)

### v135–v137 — appearance-based visual-consistency loss, layered onto the v102/v133/v134 bases

New loss `w_visual_consistency` (`models/rcf_model.py`'s `get_visual_consistency_loss`, Choudhury et al. 2021 NeurIPS eq.4 — penalizes each mask channel's pixels for deviating from that channel's own mask-weighted mean colour, computed at full image-crop resolution not mask resolution, so it doesn't throw away the pixel-colour detail it depends on). Default weight 0 → zero behaviour change everywhere it isn't explicitly set. v135 = v102+this loss, v136 = v133+this loss, v137 = v134+this loss — same single mechanism tested on three different architecture bases. **All three written for review, not submitted** .

### v138–v139 — replacing blind offset regression with explicit local correlation in the joint-mask attention line

v124/v130's `DeformableCrossFrameAttention` predicted a lookup offset purely from the query's own content (never checking what's actually at the target location — "blind"). v138 (`RCFJointMaskV5SoftTissueModel`, `models/local_correlation_cross_frame_attention.py`) replaces this with an explicit local-window content correlation (radius=4, 9×9=81 candidates, `F.unfold`-vectorized, query/key/value projected to 32ch first) — the same correspondence-finding principle as classical block-matching / FlowNet's Correlation Layer / PWC-Net's cost volume, verified real forward+backward at 18.38GB peak GPU memory (well within budget — full O(N²) attention at feat0's 96×96 had already been ruled out earlier at ~43GB estimated). v139 (`RCFJointMaskV6SoftTissueModel`) adds v130's learned query-side position embedding on top of v138's correlation mechanism.

### v140–v143 — RAFT-free (JEPA line), ultimately abandoned

Motivated by a bottleneck diagnosis (`script/diagnose_v102_bottleneck.py`, run against v102's champion checkpoint): mask miss-rate on true foreground pixels correlates strongly with RAFT's own cycle-consistency confidence (miss rate ~0.27–0.31 when confidence <0.6 vs 0.08 when ≥0.8) — RAFT's synthetic-data domain-transfer error onto endoscopic footage looked like a bigger error source than anything in the appearance/mask branch itself. v140 tried removing RAFT entirely, replacing it with a self-taught, in-network correlation-based flow (`models/local_correlation_flow_head.py`).

**Collapse history, each with a distinct diagnosed mechanism, escalating fixes, ultimate abandonment:**
- v140 first submission (job 506, gap1-only data): flow collapsed to ~0 within ~20 steps — soft-argmax's near-random-init output is ~0 by construction, and gap1's small true motion made zero flow a genuinely low-error (if trivial) solution. Fixed with gap1+2+3 mixed data + a baseline-relative reconstruction loss (reward *improvement* over a detached zero-flow baseline, not absolute error).
- v141 = v140 + grasp10 data (single-variable, same pattern as v133/v134).
- v142 (`models/rcf_jepa_flow_model.py`): flow branch kept sliding toward mask collapse (one dominant channel by step ~200–600) even after the zero-flow fix. Root cause traced to the reconstruction target being produced by the SAME backbone computing the prediction ("grading its own homework"), compounded by the aperture problem (near-zero total reconstruction error doesn't pin down a unique flow field in texture-poor regions). Fixed with a JEPA/BYOL/DINO-style EMA target encoder (`backbone2_ema`, stop-gradient) feeding only the key/value side of the correlation, context `backbone2` still feeding the mask branch and the query side. Stabilised the *flow* branch's own collapse but the *mask* branch kept collapsing on essentially the same timeline — proof the EMA-target trick wasn't the whole story.
- v143 — a fully faithful JEPA redesign (`models/jepa_motion_predictor.py`'s `JepaMotionPredictor`): no explicit flow field at all: a BLIND predictor guesses what the EMA target encoder would produce for the other frame from context features alone (no correlation, no search, no warping — v142 was only a partial JEPA adoption in this sense). Mask trained via `_predict_error_consistency_loss` (predictor residual error — high where content is genuinely unpredictable from blind context alone — as the segmentation-relevant signal).

**Net verdict (discussed at length 260729, before this line was abandoned):** every fix addressed a real, correctly-diagnosed collapse mechanism, but the *next* one always appeared — batch-marginal entropy balance regularizers got "gamed" via whole-image channel assignment; adding a per-sample balance regularizer on top made the network find an even more trivial joint solution (a literal near-constant mask). Root-caused as structural: JEPA's target signal is ultimately self-referential (even EMA-decoupled, it's a lagged copy of the same trainable system), prone to internally-consistent-but-meaningless solutions — unlike RAFT flow (an external, non-trainable, real signal). **Abandoned in favour of staying on v102's proven RAFT-based recipe** and adding new signals (starting with DINO graph-partitioning, below) as additive extensions instead of a from-scratch redesign.

---

## v144–v155 — Image-edge boundary weight, DINO graph-partitioning fusion, resolution upgrade, robust background-flow removal (260730–260802)

Design principle carried through this whole arc, stated explicitly by the user after the JEPA line was abandoned: *"don't lose what's already there — add this newly extracted information in, combine it together to do the segmentation"* (don't lose what's already there — add new information as a genuine addition, not a replacement). Every mechanism below is opt-in (default off, zero behaviour change for v102/every prior config) and, except where noted, provably or near-exactly a no-op at initialization (usually via a zero-initialized last conv layer in a residual/gating branch) — the training run starts at v102's own optimum, not a cold-started new architecture.

### v144–v145 — image-edge-derived boundary weight (replaces/augments HQAM)

`models/flow_aggregation_head_image_boundary.py`'s `FlowAggregationHeadImageBoundary`: HQAM's boundary-supervision mask (`detect_flow_changes_batch`) is keyed off RAFT's own flow-angle discontinuities — inherits RAFT's domain-transfer noise on this footage (see v140's motivating diagnosis above). An image-edge signal (guided-filter-smoothed, specular-highlight-excluded Sobel on the crop image itself, continuous not thresholded — pipeline tuned empirically against `saved/edge_test_260729/guided_sobel_test.jpg`) doesn't depend on RAFT at all.
- **v144** (`boundary_mode: image_edge`): full replacement of HQAM.
- **v145** (`boundary_mode: union`): `max(image-edge weight, HQAM's own detection)` — keeps both signal sources.

| ver | val_miou_sum peak | test_miou_frame_avg |
|---|:---:|:---:|
| v144 | 134.0 (ep9) | 69.66 |
| v145 | **138.0** (ep10) | **73.79** |

**v145 (union) clearly beats v144 (replace)** — HQAM's RAFT-derived detection still carries real information the image-edge signal alone doesn't cover, despite inheriting RAFT's noise. Union > replacement.

### v146–v155 — DINO graph-partitioning eigenvector fusion, six independent mechanisms/injection points

Grounded in 2408.14789v3 ("Revisiting Surgical Instrument Segmentation Without Human Intervention: A Graph Partitioning View") — a frozen DINO ViT's patch features, turned into a cosine-similarity affinity graph and spectrally decomposed (normalized graph Laplacian, Normalized-Cut theory), give eigenvectors whose low-eigenvalue end carries clean semantic-module structure. Empirically validated first (`saved/edge_test_260729/spectral_test.jpg`) that this pipeline gives clean instrument silhouettes on **DINO** features but pure noise on this project's own trained **ResNet50** features at the same affinity/Laplacian/eigh pipeline — DINO, not the project's own backbone, must be the feature source. Validated configuration throughout: 384×384 input (matches backbone2's own crop resolution) → patch8 → 48×48 tokens → downsampled to a 32×32 (1024-node) graph.

| ver | mechanism | injection point | trainable? | val peak | test_miou |
|---|---|---|:---:|:---:|:---:|
| **v146** | `DinoGraphFusionHead` (`models/dino_graph_fusion.py`): eigenvectors → 1×1 proj → concat → 3×3 conv, zero-init last layer | feat0 (backbone2 stage-1, 256ch, 96×96 — the LOCAL/fine-boundary-detail input) | yes | 137.0 | 73.55 |
| **v149** | same class as v146, unmodified | feat3 (backbone2 stage-4, 2048ch, 48×48 — the deepest/most-semantic scale, fused before feat0 is even consulted) | yes | **139.0** | **74.57** |
| **v147** | `DinoGraphEStepFusion` (`models/dino_graph_estep_fusion.py`): self-referential centroid matching, `P_fused ∝ P_CNN·P_Graph`, NO learned params — per channel, weighted-average the eigenvector embedding by the CNN's own current mask to get μ_k, then re-derive P_Graph from each pixel's distance to every μ_k (Gaussian/Geman-McClure-style soft weight, scale = per-sample robust MAD of distances) | decode_head2's OUTPUT (the softmax mask itself, before decode_head consumes it) | no (0 trainable params) | 137.0 | 69.71 |
| **v148** | `FlowAggregationHeadGraphMotion` (`models/flow_aggregation_head_graph_motion.py`): same self-referential-centroid math as v147, but `GraphWeight = max_k P_Graph` (graph's own top-1 confidence) multiplied directly into `mask1_agg`/`mask2_agg` — the actual weighted-least-squares MOTION FIT itself, not the mask | inside `decode_head`'s `_compute_agg_masks` (new overridable hook, small behaviour-preserving refactor of `flow_aggregation_head_with_residual_v2.py`) | no (0 trainable params, `motion_weight_floor=0.2` numerical-stability floor) | 133.0 | 67.52 |
| **v153** | `DinoGraphAttentionGate` (`models/dino_graph_attention_gate.py`): single-channel spatial gate `A=tanh(gate_logit)` (zero-init last layer), `F'=F·(1+A)` — multiplicative modulation, never injects new content, only rescales existing feat3 values | feat3 (same point as v149, different fusion TYPE — concat vs multiplicative gate) | yes | 126.0 | **73.37** |
| **v154** | v149's exact mechanism, TWO independent instances | feat2 (1024ch) **and** feat3 simultaneously (multi-scale injection) | yes | 131.0 | 68.28 |
| **v155** | `MultiScaleSegHeadDecoderPrior` (`models/multi_scale_seg_head_decoder_prior.py`) + extraction-only `DinoGraphEigvecExtractor` (`models/dino_graph_eigvec_extractor.py`, 0 params) — injected via a new `_inject_decoder_prior` hook added to `MultiScaleSegHead.forward_features` (behaviour-preserving refactor, base class default = no-op) | DECODER stage — right after decode_head2's own ASPP/multi-scale fusion, before feat0 concat (not an encoder-side backbone feature at all) | yes | 113.0 (ep**0**, then declined to 81.2 final) | 67.11 |

**Findings:**
- **v149 (feat3, concat) is the best of the DINO-graph line on both metrics** — feat3's role (deep/semantic decision layer) matches what DINO's *global* structural signal is architecturally suited for, much more than feat0's role (local boundary refinement, v146) — confirmed post-hoc by inspecting v149's actual trained weights (`fuse_conv.weight` norm moved from exactly 0 at init to 10.17 after 38 epochs — the network genuinely learned to use this path, not left it inert).
- **v153 (attention-gate) has the best test_miou_frame_avg (73.37) despite the lowest val_miou_sum peak (126.0)** among this batch — a reminder that `val_miou_sum` (oracle union, per-epoch monitoring) and `test_miou_frame_avg` (same oracle mechanism, run once at the end — see "Oracle mechanism" note below) don't always move together; single-number rankings from either alone can mislead.
- **v154 (feat2+feat3 double injection) is clearly worse than v149 (feat3 alone)** — "more injection points" did not compose additively, mirroring this project's much earlier Phase 9 finding (v87/v88/v89: independently-motivated mechanisms don't necessarily stack).
- **v155 (decoder-stage injection) shows a real, large collapse** (val peak at epoch 0, declining to 81.2 by the end) — not just noise; this specific mechanism/injection-point needs rethinking before it's trusted, not adopted as a negative-but-final result.
- **v148 (graph weight into the motion least-squares fit) is the weakest of the batch** — the most structurally novel/deepest injection point (directly into the core motion-reconstruction mechanism the whole project's contribution rests on) but currently the worst empirical result; may need different hyperparameters (`motion_weight_temperature`/`motion_weight_floor`) rather than being a dead end.

### v150 — resolution upgrade (96×96 → 192×192), independent of the DINO-graph line

`MultiScaleSegHeadHiRes` (`models/multi_scale_seg_head_hires.py`): decode_head2's output has always been capped at H/4 (96×96 for a 384 crop) — feat0 itself is already at that resolution, and GT flow is downsampled 4× before the warp loss ever sees it. Uses backbone2's stem activation (conv1+norm1+relu, BEFORE maxpool — a stride-2, 64ch feature computed on every forward pass regardless, but discarded today since `out_indices` only keeps the 4 res_layer outputs) as a genuine higher-resolution detail source, captured via a zero-cost forward hook (`RCFModel.__init__`). Bilinear-upsamples decode_head2's H/4 feature to H/2, fuses in the stem feature via a zero-init residual. Provably (not just empirically) exact no-op at init: `conv_seg` is a 1×1 conv (linear + per-channel constant bias), which commutes exactly with bilinear interpolation — verified numerically at 8.9e-08 (float32 noise floor). `mask_size`/`decode_head.mask_size` both changed to `[192,192]`; GT flow now only downsampled 2× (384→192) instead of 4×.

Result: val peak 136.0, test_miou_frame_avg 71.62 — no clear win over v102, suggesting the 96×96 bottleneck may not be the dominant limiting factor right now (or this specific implementation needs more tuning) — inconclusive, not a clean negative.

### v151–v152 — robust background-motion removal (IRLS), independent of the DINO-graph line, built on v149's base

`decode_head`'s `_bg_flow` (`models/flow_aggregation_head_with_residual_v2.py`) estimates dominant camera/background motion from GT flow and subtracts it before the per-channel motion fit, so the fit only has to explain the instrument's own residual motion. `_estimate_bg_flow_affine`'s own pre-existing docstring admitted a real weakness: the affine (rotation/scale/shear) part was fit via a single plain-OLS pass over all pixels equally weighted — "not robust to instrument pixels biasing A". New `bg_affine_irls_iters` param (default 0 = exact prior behaviour, verified unchanged for v79, the only pre-existing config using this path): when >0, iteratively refits the SAME weighted-least-squares solver, each round downweighting large-residual pixels (Gaussian/Geman-McClure soft weight, scale = per-sample robust MAD of residuals — no fixed magnitude threshold to hand-tune).
- **v152** = v149 + `bg_removal_affine: true` (plain OLS affine, NOT IRLS-robustified) — isolates whether the richer affine model helps at all.
- **v151** = v149 + same + `bg_affine_irls_iters: 3` (IRLS-robustified).

| ver | val_miou_sum peak | test_miou_frame_avg |
|---|:---:|:---:|
| v152 (plain affine) | **140.0** (ep11) | 71.87 |
| v151 (IRLS affine) | 138.0 (ep16) | 71.55 |

**Surprising: v152 (not robustified) edges out v151 (IRLS) on both metrics.** v152's 140.0 val_miou_sum peak matches/slightly exceeds the historical v102 champion range (139.77/140.12) — the highest peak recorded across the whole v144–v155 batch — though it settles lower (128.0) by the end, same "early peak, later plateau" pattern seen throughout this project's history (e.g. v75, v91 "online curve overstates the result"). Whether IRLS genuinely hurts, or the difference is within this project's typical checkpoint-to-checkpoint noise, is not yet disentangled — worth a repeat/larger sweep before concluding IRLS is net-negative.

### Sliding-window eval merge strategy (independent variable, tested only on v102's existing champion checkpoint — no retraining)

`main.py`'s `_sliding_window_eval` averages softmax probabilities across overlapping windows — implicitly assumes "window A's channel k" and "window B's channel k" mean the same thing, an assumption training never enforces (this architecture's channels have no fixed external semantics anywhere, confirmed by re-reading `main_tissue.py`'s own oracle mechanism, see below). New `_sliding_window_eval_nearest` (opt-in via `sw_nearest_center`, default off) instead assigns each pixel WHOLESALE to whichever window's centre is nearest — no blending at all (same principle as the original U-Net paper's overlap-tile strategy: trust only each tile's well-contextualised centre). Since window positions form a regular grid, this reduces to non-overlapping rectangular regions per window (midpoint-of-centres partition), no per-pixel distance computation needed.

**Result (260810, `script/run_grasp0_eval_v102_sw_nearest.sh`, real v102 champion checkpoint — the first run of this script accidentally picked up a different, later v102 retraining's `last.ckpt` via a `saved/` directory glob and gave misleadingly low numbers for both variants; re-run pinned to the actual champion checkpoint `saved_discrete_data/grasp0_tissue_ft_v102_260709_192435/epoch=27-step=10892.ckpt`): nearest-center is WORSE — 140.19 (averaging, current default) vs 138.61 (nearest-center), −1.58, mostly from tissue (74.57→73.18) not instrument (65.62→65.43).** Tissue boundaries are blurrier/more diffuse than instrument's, so the multi-window soft blending this mechanism removes was doing real, disproportionately tissue-side work. **Conclusion: keep the existing probability-averaging merge; nearest-center is a validated-negative, closed result, not pending.**

Two follow-up eval-side ideas were also considered but not implemented this session: (1) matching eval's resize target to train's per-source-resolution branching instead of one universal ~392 target (tested as `rcf_cmc_grasp0_tissue_ft_v102_resaware_eval.yaml`, eval-only, no retrain) — **verified NEGATIVE, 140.19→133.16**, because the champion checkpoint's own per-epoch validation (which drives which checkpoint gets kept as "best") was itself always run under the old fixed-392 scale; patching eval alone without retraining just breaks that calibration instead of fixing anything real. This motivated v156/v157 below (fix the training side instead). (2) multi-scale test-time augmentation (evaluate at 2-3 scales, ensemble) — discussed as a cheaper alternative to retraining but not tried.

### Oracle mechanism — clarified this session (applies to `val_miou_*` AND `test_miou_frame_avg` identically)

`main_tissue.py`'s `TissueModel.validation_step` resets `self.object_channel = None` before every validation call, forcing `main.py`'s `test_step` into oracle mode (`_in_oracle_mode = object_channel is None`) — and since `TissueModel.validation_epoch_end` has its own complete dual-val aggregation logic that never calls the base class's "auto-detect and lock a fixed channel" code path, `object_channel` never gets set to a fixed value anywhere in this codepath, including for the FINAL test phase (`trainer.test(ckpt_path='best')`, called in the same process right after `trainer.fit()` — same model instance, `test_step` inherited unchanged from `main.py`). **Both `val_miou_sum` (monitored every epoch) and `test_miou_frame_avg` (the final reported number) are therefore the same per-frame, exhaustive-subset-union oracle** (main.py:602-645) — for each frame independently, brute-force try every non-empty subset of the 5 mask channels (minus any `oracle_exclude_channels`), keep whichever union gives the highest IoU. No requirement that the same channel(s) mean the same thing across frames. This is not a fixed-channel "deployment-realistic" evaluation on either metric — both are equally "best-case, given this prediction" numbers.

### grasp10 tissue annotation completion (260810)

`annotations_108/instances_default.json` (103 grasp10 images, instrument-only — tissue labelling was explicitly left pending, rendered as an all-black placeholder by `tools/render_coco_masks_grasp10.py`) received real tissue annotations (`annotations_108_new/instances_default.json`, same 103 stems verified identical, now 99 tissue polygon annotations vs 0 before). Script re-pointed at the new file and re-run: `masks_all/`'s tissue-mask non-empty count went from 103/206 to 202/206. A new `val_both200.txt` (200 stems with BOTH instrument and tissue non-empty — not 202, since 2 of the newly-tissue-labelled stems turned out to have empty instrument masks) was generated in both `eval_instrument/ImageSets/` and `eval_tissue/ImageSets/`, alongside (not replacing) the old `val_both101.txt`. Pre-change state backed up to `CMC_grasp10_deinterlaced/_backup_before_tissue108_update_260810_085628/`. No config currently points at `val_both200.txt` yet — needs updating in any future grasp10-joint-eval config that currently references `val_both101`/`val_paired_both101`.

**Follow-up (260810), matching grasp0's own convention**: grasp0's `eval_instrument`/`eval_tissue` `val.txt` files are each independently filtered to 100% non-empty for their OWN category (213/209, 0 empty either way — verified by reading every mask pixel, not assumed). grasp10's `val.txt` (206, the raw annotation union) was NOT filtered this way — it included 4 all-black instrument masks and 4 all-black tissue masks (2 stems all-black in both). Fixed to match: `eval_instrument/ImageSets/val.txt` and `eval_tissue/ImageSets/val.txt` independently filtered to non-empty-only, **206→202 each** (backed up to `CMC_grasp10_deinterlaced/_backup_before_valtxt_filter_260810_105146/`). Downstream: `train_g0378_g5592_g10388_clean_holdout.txt`'s g10 exclusion set was built against the OLD 206-stem union — recomputed against the correct exclusion criterion (union of the CURRENT 202+202, i.e. 204 stems, not 206) and found 2 stems over-excluded (the 2 now-empty-in-both stems, no longer real eval cases) → new `train_g0378_g5592_g10390_clean_holdout.txt` (g10: 388→390, g0/g5 unchanged), old 388-file kept alongside, not overwritten.

## v156–v162 — Train/eval scale mismatch, EMA, bg_removal_affine root-cause, DINO Bayesian prior (260810–260811)

### Train/eval scale mismatch, traced and fixed at the training side

Discussion (260810) found a real, quantifiable gap: training's `ResolutionAwareCrop` branches by source resolution (576p → resize_short 400, 1080p → resize_short 576), but eval uses ONE universal target (~392) regardless of source. For 1080p sources specifically this is a real ~1.47× scale mismatch (train sees objects at ~0.53× scale, eval was showing them at ~0.36×; 576p sources have almost none, 400 vs 392). Patching eval alone (see sliding-window section above) made things worse, not better — the champion checkpoint's own selection was itself calibrated under the old eval scale. **v156/v157 fix the training side instead**: new `MultiScaleCrop` (`dataset/transforms.py`) replaces the two narrow resolution-gated branches with ONE wide, continuous random-scale range (`resize_short=480, ratio_range=[0.73,1.25]`) applied uniformly regardless of source resolution — standard Detectron2/YOLO-style multi-scale training, aiming for scale ROBUSTNESS instead of hand-matching one eval number. (A pre-existing `RandomCrop` safety net rescues any draw whose target short side falls below `crop_size`=384 back up to exactly 384 — the effective range is really ~384–600, not the full 350–600 originally intended, with a probability spike exactly at 384.) `epochs` raised 80→120 to compensate for the wider distribution needing more iterations to converge (based on v102's own historical checkpoint-retention pattern — top-3 checkpoints cluster around epoch 50 of an 80-epoch run, ~30 epochs of headroom already existing before this change).
- **v156** = v102 + `MultiScaleCrop`.
- **v157** = v152 + `MultiScaleCrop` (+ epochs 120) — deliberately built on v152 (the DINO-fusion batch's val-peak leader), not v149, to test the mechanism on the strongest available base.

Eval batch (`script/run_grasp0_eval_v156_v162.sh`) submitted 260817; results pending at time of writing.

### EMA (exponential moving average weights) — first real use since v80

This project has a complete, working EMA mechanism (`models/rcf_model.py`: `create_backbone_with_ema`/`create_decode_head_with_ema` + `momentum_update_param_and_buffer` every training step, `eval_on_ema` switches both validation and the sliding-window test path to the EMA weights) that was built early in the project's history but only ever tried once (v80 = v64+EMA) with an inconclusive negative read (README flagged that check itself as premature — "EMA lags main weights early on... needs re-checking once training resumes/completes" — never followed up). Motivation to retry now: v156/v157's `val_miou_sum` curves oscillate heavily epoch-to-epoch (~124–134 band, single-epoch swings of 8–10 points), typical for this dataset scale + wide augmentation, and EMA is the standard (Mean Teacher/BYOL) "smooth the noisy online signal for free" fix.
- **v158** = v156 + EMA (`eval_on_ema: true`, `ema_m: 0.999`).
- **v159** = v157 + EMA — DINO-fusion interaction verified by direct code read (not assumed) before building: both `forward_eval`'s `eval_on_ema` branch and its non-EMA branch independently call `dino_graph_fusion_deep_head` on whichever feat3 that branch just extracted — no conflict, though the DINO fusion module's own weights are NOT themselves EMA-tracked (only backbone2/decode_head2 get an EMA twin) — a partial-EMA setup, expected and documented, not a bug.

Eval batch submitted alongside v156/v157 above; results pending.

### bg_removal_affine root cause: traced to a real, pre-existing project-wide tension, not a v151/v152-specific bug

User observation ("why did v157 actually get worse instead") prompted pulling v149's and v152's FULL 81-epoch `val_miou_instrument`/`val_miou_tissue` curves (not just the sparse saved-checkpoint samples used for batch-eval tables) directly from their tensorboard logs. Finding: **v149/v102 (bg_removal_affine=False) show noisy-but-flat instrument mIoU across all 80 epochs (~50–67%, no trend). v151/v152 (bg_removal_affine=True, IRLS-robustified or plain-OLS) BOTH show a distinct "early peak (~63–68%, epoch 8–20) then decline to a ~50–57% plateau for the rest of training" shape** — isolating the affine background-motion-removal mechanism itself as the cause (not DINO fusion — v149 has it without the decline; not specifically IRLS's robustification — v151's IRLS shows the same decline shape as v152's plain OLS). Root cause, per `_estimate_bg_flow_affine`'s own pre-existing docstring: the affine fit is "not robust to instrument pixels biasing A" — and critically, IRLS's residual-magnitude-based downweighting structurally under-corrects this, because a rigidly-translating instrument has smooth INTERNAL flow (no local residual spike relative to its own motion, only at its silhouette edge) — IRLS can only ever catch the edge, never the interior.

Also clarified: this "early peak then plateau/decline" shape is not unique to bg_removal_affine — README already documents a milder version of the same pattern at v75/v91/v102 ("online curve overstates the result"). bg_removal_affine amplifies a pre-existing, project-wide tendency rather than introducing a new failure mode from nothing.

### v160 — EMA-teacher-mask-weighted background-affine fit: negative result

Attempted fix for the above: use an EMA teacher's own instrument-channel prediction as a soft, whole-silhouette background-confidence weight (`1 - P_ema(instrument)`) fed directly into `_estimate_bg_flow_affine`'s fit (new `set_ema_bg_weights`/`exclude_weight` param, composable with IRLS — IRLS's residual reweighting starts FROM this weight instead of all-ones). Semantically targets the exact gap IRLS can't reach (the instrument's whole interior, not just its residual-spike edge). Safeguards against self-bootstrapping: EMA not live student (no direct gradient path back into the excluding signal), SOFT weight not hard threshold, `bg_mask_exclude_start_epoch=10` warm-up (decode_head2_ema is meaningless before the EMA teacher has learned anything). Built on v152 (`decode_head.use_ema_bg_mask: true`).

**Result: negative.** Epoch10–43 average vs v149/v152 over the same window: v149 131.66, v152 129.97, **v160 125.03** — worse than both, and its instrument mIoU never exceeded 59.60 (v152 reached 67.72, v149 65.19). The mechanism appears to actively hurt rather than help — plausibly because `decode_head2_ema` at epoch 10 (the warm-up cutoff) is still too unreliable, and an imprecise exclusion weight dilutes the affine fit's effective sample count/quality in a different bad way rather than cleanly removing contamination. Not retried with a later warm-up epoch at time of writing — the direction's cost/benefit looked poor enough after this one data point to deprioritize rather than immediately iterate.

### v161–v162 — DINO graph-partitioning as a genuine Bayesian PRIOR (decision-layer), not a feature-layer concat fusion

Reframing (260811 discussion): this architecture is already EM-style motion clustering (mask = E-step soft responsibility, per-channel rigid-motion fit = M-step, `warp_seg` drives both). Under that framing, DINO's eigenvectors are a genuine, motion-INDEPENDENT prior `P(channel | appearance)`, and the project already has (but had never enabled) a genuine motion-derived likelihood: `_em_consistency_loss`'s `target = softmax(-per_channel_rigid_motion_error / T)`. Bayes' rule combines these by ADDING their logs. This is architecturally different from v146/v149/v152's `DinoGraphFusionHead` (concatenates eigenvectors into a mid-network CNN feature, feat0 or feat3, then lets several more conv/ASPP layers opaquely reprocess them before any decision) — the new mechanism instead computes a genuine `C`-way categorical distribution directly from DINO and adds its log-probability straight onto `decode_head2`'s raw mask logits, before the final softmax.

2408.14789v3 re-read in full for this (not just skimmed for the affinity/Laplacian recipe as in the v146–v155 arc) — **correction to this project's earlier informal description**: individual eigenvectors are NOT per-pixel class probabilities. Each is a single real-valued partition "mode" at a specific normalized-cut granularity (v1/v2 = coarsest/most salient split, e.g. cleanly separates instrument from background; higher-index eigenvectors = progressively finer sub-structure, becoming noise-like past roughly g~10–15 for binary/part-level tasks per the paper's own Table 6 ablation — this project's existing `num_eigvecs=10` default lands exactly in that paper-validated range, not an arbitrary choice). The paper's own recipe for turning eigenvectors into an actual segmentation is to stack the first g into a g-dim per-pixel EMBEDDING and run K-MEANS on it (Sec 3.5) — NOT to read class membership off raw eigenvector values directly.

New `DinoGraphBayesianPrior` (`models/dino_graph_bayesian_prior.py`) follows that recipe with learnable (gradient-trainable) centroids instead of an offline K-Means pass: soft squared-distance to `C=mask_layer` centroids in the 10-dim eigenvector-embedding space, temperature-scaled — the SAME mathematical form as the motion side's `softmax(-error/T)` likelihood, so prior and likelihood are dimensionally/functionally symmetric before summing. Output scaled by a zero-init learnable scalar (exact no-op at init, same discipline as every other DINO-graph module this project has added).
- **v161** = v102 + `DinoGraphBayesianPrior` (replaces, not stacks with, v149's concat fusion) + `_em_consistency_loss` enabled (`weight: 0.1`) — built on v102 (not v152), deliberately isolating this new mechanism from the known bg_removal_affine instrument-decay confound documented above.
- **v162** = v161 + a design gap found while explaining v161 to the user: `_em_consistency_loss` as originally written does an unweighted spatial mean over every pixel, disagreeing with `warp_seg`'s own `boundary_dilation`/`floor` weighting (concentrated near motion-boundary pixels) about where the signal should matter. New `em_consistency_use_boundary_weight` param (default False, exact-prior-behaviour when off) makes it reuse the SAME `mask_fw_flow`/`mask_bw_flow` boundary weight `warp_seg` already computes.

Eval batch submitted alongside v156–v160; results pending at time of writing.
