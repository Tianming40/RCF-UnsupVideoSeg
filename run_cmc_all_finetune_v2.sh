...
#SBATCH --job-name=rcf_cmc_all_v2
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=16
#SBATCH --mem=64G
#SBATCH --time=24:00:00
#SBATCH --output=slurm_rcf_cmc_all_v2_%j.out
...
echo "=========================================="
echo "Fine-tuning V2 on CMC grasp-0+5+10 merged (topk=4, flow=_NewCT, 1443 train pairs)"
echo "Started at: $(date)"
echo "=========================================="

TIMESTAMP=$(date +%m%d_%H%M%S)
RUN_DIR="saved/saved_cmc_all_finetune_v2_${TIMESTAMP}"

echo "Checkpoint dir: ${RUN_DIR}"

CUDA_VISIBLE_DEVICES=0 python main_v2.py \
    configs/instrument/rcf_cmc_all_finetune_v2.yaml \
