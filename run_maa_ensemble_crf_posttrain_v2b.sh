#!/bin/bash
# MAA + Flow weighted inference ensemble — CRF post-trained checkpoint
# Base model  : saved_cmc_all_finetune_v2b_0528_132020/epoch=17 (65.21% best)
# Post-trained : saved_cmc_crf_posttrain_v2b_0601_110702/epoch=1  (val 62.93% best)
# Architecture : V2b (mask_size=128, clamp_flow_t=10, free_residual_with_affine)
#
# Usage:
#   CUDA_VISIBLE_DEVICES=0 bash run_maa_ensemble_crf_posttrain_v2b.sh
#
# Optional overrides (pass as env vars):
#   ALPHA=0.7   bash run_maa_ensemble_crf_posttrain_v2b.sh   # more weight on MAA
#   ALPHA=0.3   bash run_maa_ensemble_crf_posttrain_v2b.sh   # more weight on flow
...
CKPT="saved/saved_cmc_crf_posttrain_v2b_0601_110702/epoch=1-step=722.ckpt"
CONFIG="configs/instrument/maa_ensemble_v2b.yaml"
TIMESTAMP=$(date +%m%d_%H%M%S)
OUTPUT="saved/maa_ensemble_crf_posttrain_v2b_${TIMESTAMP}"

# MAA-flow balance: 0.5 = equal weight, 1.0 = MAA only, 0.0 = flow only
ALPHA=${ALPHA:-0.5}

echo "=========================================="
echo "MAA + Flow Ensemble — CRF post-trained checkpoint"
...
# TTA_FLAG="--tta"
# CRF_FLAG="--use_crf"
# EXTRA="--extra_ckpts saved/saved_cmc_crf_posttrain_v2b_0601_110702/last.ckpt"
