#!/usr/bin/env python3
"""
Bob deinterlacing for CMC grasp-0 images (same field order as grasp-5/10).

Even rows (0,2,4...) = OLD field → copy from odd row below.
Odd rows  (1,3,5...) = NEW field → keep.

Input:  CMC_grasp0_from_raw/JPEGImages/<STEM>/
Output: CMC_grasp0_deinterlaced/JPEGImages/<STEM>/

Usage:
  python tools/deinterlace_cmc_grasp0.py
"""
...
SRC_ROOT = Path("/media/mitiadmin/Micron_7450_1/tianming/dataset/CMC_grasp0_from_raw")
DST_ROOT = Path("/media/mitiadmin/Micron_7450_1/tianming/dataset/CMC_grasp0_deinterlaced")
...
def main():
    ...
    for src_path in tqdm(all_pngs, desc="Deinterlacing"):
        ...
        Image.fromarray(bob_deinterlace(img)).save(dst_path)
    ...
    print(f"\nOutput: {DST_ROOT}")
    print("Next step: bash run_flows_grasp0.sh")
